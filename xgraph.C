...
//
// Ausrechnen, wieviele Planes ben�tigt werden
//
	n = nbg_cols;		// Planes f�r Hintergrund sammeln
	while( n )	{ nplanes++;	n>>=1; }
...
/*
	...
	if(XMatchVisualInfo (dpy, DefaultScreen(dpy), 24, DirectColor, &matchedVisual) == 0) {
		printf("============================= NO Matched Visuals\n");
	} else {
		printf("============================= Visual Matched\n");
	};

	printf("Creating color map\n");
	cmap = XCreateColormap(dpy, win, matchedVisual.visual, AllocNone);
*/
...
//
// Gr��e des Fensters bestimmen: Fenster zentrieren
//

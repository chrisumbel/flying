...
// - flackerfreies Bewegen der B�lle durch Hilfspixmaps
...
int	r_len;							// Zahl gueltige Bits an Lesezeigerposition
	...
	r_len   = (len*ws)%8;			// g�ltige Bits im letzen Byte
...
int		x,y;
double	dr=r-0.5;		// tats�chlicher (Welt-)Radius
...
int absy = (dy>0)?dy:-dy;
	...
	if ( (absx<d_help-d) && (absy<d_help-d) ) {
		...
		int	width = d+absx;		// tats�chlicher Ausschnittgr��e der Pixmap
		int	height= d+absy;
...
struct lightdata {
	...
	for (y=0;y<d;y++) {							// F�r alle Punkte der Bitmaps
		for (x=0;x<d;x++) {
			double dx = x-dr;								// X-Abstand zur Kreismitte
			double dy = y-dr;								// Y-Abstand zur Kreismitte
			double dist2 = dx*dx + dy*dy;
			if (dist2<max_rad2) {
				...
				if (dz>=0) {
					...
					for (int px=0;px<lpixs_x;px++) {	// f�r alle Bitmaps
...
//
// Zahl der Pixmap's, Gr��e der Felder bestimmen
//
	sym = (mode>1)?2.0:1.0;
	//sym = 1.0;

	if (mode) {
		rpixs_l   = (int)(3.14*d/sym);   // Zahl der Bitmaps in L�ngen-Richtung
	...
	vecs_all  = vecs_l * vecs_b;			// Gesamtgr��e des Rasters
...
FILE	*fp;
	...
	for (l=0;l<rpixs_l;l++) {				// f�r alle Bitmaps
		for (b=0;b<rpixs_b;b++) {
	...
	for (y=0;y<d;y++) {
		for (x=0;x<d;x++) {
			...
			if (dist2<max_rad2) {
		//		double dzr = sqrt(dr*dr-dy*dy);		// Kugel-Radius auf H�he z
...
Real	b_ang = (double)mult*M_PI/vecs_b;	// Rotationswinkel pro Pixel
	...
	if (debug&ShowRings) {
		...
		XCopyArea(dpy,bpix,nbpix,gc,0,0,d,d,o/2,o/2);	// Ball l�schen
		XFreeGC(dpy,gc);
	}
#endif
